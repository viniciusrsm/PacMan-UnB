...
BITMAP_POS_INICIAL:	.half 148, 177
MATRIZ_ATUAL:		.word 0
MAPA_ATUAL:			.word 0
...
VITORIA:
		...
		li t0,10
		bne t2,t0,VITORIA	# se não for enter volta para loop
...
INPUT_TECLADO:
		...
		jal s8 TROCA_SPRITE_ATUAL		# se o tempo atual formatado for = 0 -> JUMP_MOVIMENTO_ATUAL
		
		slli s1, a0, 28
		srli s1, s1, 28
		
		beqz s1, MOVIMENTACAO		# se o tempo atual formatado for = 0 -> JUMP_MOVIMENTO_ATUAL
		...
		la a3, CHAR_CIMA
		li a2, -28
		beq t2,t0,STORE_PROXIMO_MOVIMENTO		# se tecla pressionada for 'w', chama CHAR_CIMA
		...
		la a3, CHAR_ESQ
		li a2, -1
		beq t2,t0,STORE_PROXIMO_MOVIMENTO		# se tecla pressionada for 'a', chama CHAR_CIMA
		...
		la a3, CHAR_BAIXO
		li a2, 28
		beq t2,t0,STORE_PROXIMO_MOVIMENTO		# se tecla pressionada for 's', chama CHAR_CIMA
		...
		la a3, CHAR_DIR
		li a2, 1
		beq t2,t0,STORE_PROXIMO_MOVIMENTO		# se tecla pressionada for 'd', chama CHAR_CIMA
...
MOVIMENTACAO:
		...
		lh s1, (t0) # s1 = x inicial
		lh s2, (t1) # s2 = x atual
		lh s3, 2 (t0) # s3 = y inicial
		lh s4, 2 (t1) # s4 = y atual
		
		sub t0, s2, s1
		sub t1, s4, s3
		
		li t2, 8
		remu t0, t0, t2 	# resto do eixo x
		remu t1, t1, t2		# resto do eixo y
		
		add t0, t0, t1
		la t3, POS_MULT_8
		sh t0, (t3)
		
		bnez t0, JUMP_MOVIMENTO_ATUAL

		la s1, SPRITE_COUNTER
		lb t0, (s1)
		addi t0, t0, 1
		sb t0, (s1)
		...
		beqz s1, JUMP_MOVIMENTO_ATUAL	# se s1 for = 0 
		
		jal s8, PRE_CHECK_MOVIMENTO		# checa o proximo movimento
		
		la t0, PROXIMO_MOVIMENTO	# t0 = endereco de PROXIMO_MOVIMENTO
		lw s1, (t0)					# t1 = word em t0 (endereco do proximo movimento)
		sw zero, (t0)				# zera o conteudo de PROXIMO_MOVIMENTO
		jr s1							# vai para o proximo movimento

TROCA_SPRITE_ATUAL:
		bnez a1, MV_SEM_COLISAO
...
STORE_PROXIMO_MOVIMENTO:
		...
		la t0, PROXIMO_MOVIMENTO	# t0 = endereco de PROXIMO_MOVIMENTO
		sw a3, (t0)					# armazena o proximo movimento valido (a3) em PROXIMO_MOVIMENTO
...
JUMP_MOVIMENTO_ATUAL:
		la t0, MOVIMENTO_ATUAL		# t0 = endereco de MOVIMENTO_ATUAL
...
CHAR_ESQ:
		...
		la t0, MOVIMENTO_ATUAL
		sw s8, (t0)					# guarda CHAR_ESQ em MOVIMENTO_ATUAL
		...
		addi t1,t1,-1			# decrementa 8 pixeis
		sh t1,0(t0)			# salva
		ret
...
CHAR_DIR:	
		...
		la t0, MOVIMENTO_ATUAL
		sw s8, (t0)					# guarda CHAR_DIR em MOVIMENTO_ATUAL
		...
		la t0,BITMAP_POS
		lh t1,0(t0)			# carrega o x atual do personagem
		addi t1,t1,1			# incrementa 8 pixeis
		sh t1,0(t0)			# salva
		ret
...
CHAR_CIMA:
		...
		lh t1,2(t0)			# carrega o x atual do personagem
		addi t1,t1,-1			# decrementa 8 pixeis
		sh t1,2(t0)			# salva
		ret
...
PRE_CHECK_COLISAO:
		...
		li t0, 1		
		beq s3, t0, MV_SEM_COLISAO			# se o destino na matriz (s3) for uma parede (1) -> FIM
		
		li t0, 8		
		beq s3, t0, MV_SEM_COLISAO			# se o destino na matriz (s3) for o portao da base fantasma (8) -> FIM
		
		la t0, MOVIMENTO_ATUAL		# t0 = endereco de MOVIMENTO_ATUAL
		sw a3, (t0)					# armazena o endereco do ultimo movimento (a3) em MOVIMENTO_ATUAL
...
PRE_CHECK_MOVIMENTO:
		...
		li t0, 1
		beq s3, t0, JUMP_MOVIMENTO_ATUAL	# se for uma parede -> continua movimento normal
...
PORTAL_ESQUERDA:
		...
		la t1,OLD_BITMAP_POS		# carrega em t1 o endereco de OLD_BITMAP_POS
		lh t2,0(t0)
		sh t2,0(t1)			# salva a posicao atual do personagem em OLD_BITMAP_POS
