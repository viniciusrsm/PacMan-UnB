.data
CHAR_POS:		.half 148,177				# x, y
OLD_CHAR_POS:	.half 0,0				# x, y

# s1 = posicao do pacman
# s2 = pontuacao total
# s3 = vidas totais
...
SETUP:		
		la t0, MATRIZ_1
		addi s1, t0, 657		# carrega posicao inicial do pacman
		la a0,maze			# carrega o endereco do sprite 'map' em a0
		li a1,0				# x = 0
		li a2,0				# y = 0
		li a3,0				# frame = 0
...
GAME_LOOP:	call KEY2			# chama o procedimento de entrada do teclado
		...
		la a0,pacman			# carrega o endereco do sprite 'char' em a0
		...
		la a0,black			# carrega o endereco do sprite 'tile' em a0
...
CHAR_ESQ:
		li t0, 1
		addi t1, s1, -1		# t1 = endereco na esquerda do pacman
		lb t2, 0(t1)		# t2 = numero localizado em t1
		beq t2, t0, FIM
		mv s1, t1			# se nao -> a posicao do pacman atualiza (s1 atualiza para t1)
		...
		addi t1,t1,-8			# decrementa 16 pixeis
		sh t1,0(t0)			# salva
		ret

CHAR_DIR:	
		li t0, 1
		addi t1, s1, 1		# t1 = endereco na direita do pacman
		lb t2, 0(t1)		# t2 = numero localizado em t1
		beq t2, t0, FIM
		mv s1, t1			# se nao -> a posicao do pacman atualiza (s1 atualiza para t1)
		...
		la t0,CHAR_POS
		lh t1,0(t0)			# carrega o x atual do personagem
		addi t1,t1,8			# incrementa 16 pixeis
		sh t1,0(t0)			# salva
		ret

CHAR_CIMA:
		li t0, 1
		addi t1, s1, -28		# t1 = endereco em cima do pacman
		lb t2, 0(t1)		# t2 = numero localizado em t1
		beq t2, t0, FIM
		mv s1, t1			# se nao -> a posicao do pacman atualiza (s1 atualiza para t1)
...
CHAR_BAIXO:	
		li t0, 1
		addi t1, s1, 28		# t1 = endereco embaixo do pacman
		lb t2, 0(t1)		# t2 = numero localizado em t1
		beq t2, t0, FIM
		mv s1, t1			# se nao -> a posicao do pacman atualiza (s1 atualiza para t1)
...
PRINT:		li t0,0xFF0			# carrega 0xFF0 (frame 0) em t0
